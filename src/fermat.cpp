...
void Fermat::run(InputNum& input, arithmetic::GWState& gwstate, File& file_checkpoint, File& file_recoverypoint, Logging& logging, Proof* proof)
{
    ...
    if (type() == PROTH && (*_task->result() == 0 || *_task->result() == *gwstate.N))
    {
        ...
        size_t digits = num.digits(10);
        char dig[16] = "";
        snprintf(dig, sizeof dig, "%zu", digits);
        logging.result(_prime, "%s is prime! (%s digits) Time: %.1f s.\n", input.display_text().data(), dig, logging.progress().time_total());
        logging.result_save(input.input_text() + " is prime! (" + dig + " digits) Time: " + std::to_string((int)logging.progress().time_total()) + " s.\n");
    }
    ...
    if (!_prime && _success)
    {
        ...
        logging.result_save(input.input_text() + " is a probable prime! (" + dig + " digits) Time: " + std::to_string((int)logging.progress().time_total()) + " s.\n");
    }
